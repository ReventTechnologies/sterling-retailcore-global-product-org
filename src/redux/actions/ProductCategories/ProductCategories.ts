...
export const getProductCategories = () => async (dispatch: Dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.get(`${ReventBaseUrl}/product-category/all/association`)
...
export const saveGPO = (gpoData) => async (dispatch: Dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.patch(`${ReventBaseUrl}/product-category/save`, gpoData)
    //  console.log(data)
    if (data?.status === 'success') {
      dispatch({
...
export const saveProductTypeName = (productTypeId: string, productTypeName: string) => async (dispatch: Dispatch) => {
  ...
  try {
    ...
    const { data } = await axiosInstance.patch(`${ReventBaseUrl}/product-type/${productTypeId}`, { name: productTypeName })
    if (data?.status === 'success') {
      dispatch({
